...
class SlidingWindowLogRedisAdapterIT extends AbstractRedisIT {
    ...
    private static RateLimitKey newKey() {
        return new RateLimitKey(TenantId.generate(), ResourceId.generate(), new ClientIp("203.0.113.20"),
                StrategyType.SLIDING_WINDOW_LOG);
    }

    @Test
    void allowsRequestsWhileTrailingWindowHasCapacity() {
    ...
    @Test
    void deniesRequestsWhenTrailingWindowIsFull() {
        RateLimitKey key = newKey();
    ...
    @Test
    void expiredEntriesArePurgedBeforeCounting() {
        RateLimitKey key = newKey();
